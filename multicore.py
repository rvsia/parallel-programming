...
def comparison(uncompared, compared):
    global codes_id
    first = -1
    ...
    while should_continue:
        ...
        if first != -1: # uncompared document found
            for second in codes_id:
                mutexCompared.acquire()
                if not second in compared: # the second document is uncompared
                    mutexCompared.release()
                    ...
                    mutexCompared.release()
            with mutexCompared:
                compared[first] = first
        else: # no uncompared document left
            should_continue = False
        first = -1

if __name__ == "__main__":
    process1 = Process(target=comparison, args=[uncompared, compared])
    process2 = Process(target=comparison, args=[uncompared, compared])
    process3 = Process(target=comparison, args=[uncompared, compared])
